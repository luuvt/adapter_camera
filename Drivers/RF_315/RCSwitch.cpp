...
#define __DEBUG__ 0
#ifndef __MODULE__ 
#define __MODULE__ "RCSwitch.cpp"
#endif
#include "RCSwitch.h"
...
/*RCSwitch* RCSwitch::m_instance = NULL;
RCSwitch* RCSwitch::getInstance()
{
	if(m_instance == NULL)
	{
		m_instance = new RCSwitch();
	}
...
int RCSwitch::nReceiveTolerance = 10;// 60
...
void RCSwitch::transmit(HighLow pulses) {
	...
	HM_BSP_RF315_DATA_PIN_HIGH();
	delayMicroSeconds(this->protocol.pulseLength * pulses.high);
	HM_BSP_RF315_DATA_PIN_LOW();
...
void RCSwitch::enableReceive() {
  if (this->nReceiverInterrupt != -1) {
    ...
    /*low lever interrupt */
		EXTI0_IRQHandler();
     // attachInterrupt(this->nReceiverInterrupt);
...
void RCSwitch::handleInterrupt() {
  ...
  if (duration > RCSwitch::nSeparationLimit) {
    ...
    // be the gap between two transmission.
		debug("duration: %d  nSeparationLimit: %d timings[0]: %d \r\n",duration,RCSwitch::nSeparationLimit,RCSwitch::timings[0]);
    if (diff(duration, RCSwitch::timings[0]) < 200) {
			debug("diff(%d , %d ): %d \n",duration,RCSwitch::timings[0],diff(duration, RCSwitch::timings[0]));
        ...
        for(unsigned int i = 1; i <= numProto; i++) {
          if (receiveProtocol(i, changeCount)) {
            // receive succeeded for protocol i
						debug("repeatCount: %d",repeatCount);
            break;
          }
        }
        repeatCount = 0;
